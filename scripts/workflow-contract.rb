...
module WorkflowContract
  ...
  def validate(workflow)
    ...
    failures << 'bound validation to twenty minutes exactly once' unless workflow.scan(/^    timeout-minutes: 20$/).length == 1
    ...
    failures << 'run the canonical full validation gate exactly once' unless workflow.scan(/^        run: make check$/).length == 1
    failures << 'not allow validation failures' if workflow.include?('continue-on-error')
    failures << 'not bypass the canonical Make gate with inline Xcode commands' if workflow.include?('xcodebuild')
